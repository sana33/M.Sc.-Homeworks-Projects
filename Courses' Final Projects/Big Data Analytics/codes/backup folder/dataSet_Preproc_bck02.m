...
misValIdx = find(autos_temp(:,2)==-inf | autos_temp(:,6)==-inf | autos_temp(:,19)==-inf | ...
    autos_temp(:,20)==-inf | autos_temp(:,22)==-inf | autos_temp(:,23)==-inf | autos_temp(:,26)==-inf);
misValNOTidx = setdiff(seqVec,misValIdx);
...
mode23 = mode(Y23);
mode26 = mode(Y26);
autosTemp_misVal(:,3) = mode1;
autosTemp_misVal(:,7) = mode2;
autosTemp_misVal(:,20) = mode3;
autosTemp_misVal(:,21) = mode1;
autosTemp_misVal(:,23) = mode2;
autosTemp_misVal(:,24) = mode3;
autosTemp_misVal(:,27) = mode3;

autos_final = [autosTemp_NOTmisVal; autosTemp_misVal];
autos_final = sortrows(autos_final,1);
autos_final = autos_final(:,2:end);

save('autos.mat','autos_final');
