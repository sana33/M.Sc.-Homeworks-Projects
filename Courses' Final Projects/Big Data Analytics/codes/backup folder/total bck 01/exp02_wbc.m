...
[OS,OSidx,Jopt,OF,UO] = ITB_SP(wbc(:,1:end-1),size(wbc,1),0);
[X,Y,~,AUC] = perfcurve(wbc(:,end),OF,4);
...
[OS,OSidx,Jopt,OF,UO] = ITB_SP(wbc(:,1:end-1),size(wbc,1),1);
[X,Y,~,AUC] = perfcurve(wbc(:,end),OF,4);
figure; plot(X,Y,'r','LineWidth',1.5); grid on;
...
[OS,OSidx,Jopt,OF,UO] = ITB_SS(wbc(:,1:end-1),size(wbc,1),0);
[X,Y,~,AUC] = perfcurve(wbc(:,end),OF,4);
figure; plot(X,Y,'r','LineWidth',1.5); grid on;
...
[OS,OSidx,Jopt,OF,UO] = ITB_SS(wbc(:,1:end-1),size(wbc,1),1);
[X,Y,~,AUC] = perfcurve(wbc(:,end),OF,4);
figure; plot(X,Y,'r','LineWidth',1.5); grid on;
