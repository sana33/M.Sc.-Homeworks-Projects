...
Opt_Jopt = zeros(10,1);
for o = 1:10
    [Opt_OSidx{o,1},Opt_Jopt(o,1)] = J_Optimal(soybeanSm,o);
end
soybeanSm_Opt = struct('Opt_OSidx',Opt_OSidx,'Opt_Jopt',Opt_Jopt);

save('exp01_soybeanSm_res.mat','soybeanSm_ITBSP','soybeanSm_ITBSS','soybeanSm_Opt');

fprintf('\n');
fprintf('-------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n');
fprintf('o\tITB-SP\t\t\t\t\t\t\t\t\t\tJx(Y,o)\t\tITB-SS\t\t\t\t\t\t\t\t\t\tJx(Y,o)\t\tOptimal\t\t\t\t\t\t\t\t\t\tJx(Y,o)\t\n');
fprintf('-------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n');
for k1 = 1:10
    bsT = '';
    for k2=1:12-k1; bsT=[bsT,'\t']; end
    fprintf(['%d:\t%s',bsT,'%0.3f\t\t%s',bsT,'%0.3f\t\t%s',bsT,'%0.3f\t\n'], ...
        k1,num2str(ITBSP_OSidx{k1,1}'),ITBSP_Jopt(k1),num2str(ITBSS_OSidx{k1,1}'),ITBSS_Jopt(k1), ...
        num2str(Opt_OSidx{k1,1}),Opt_Jopt(k1));
end
fprintf('-------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n');
