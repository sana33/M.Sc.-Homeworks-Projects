...
ds_caHepPh = [ds_caHepPh; [ds_caHepPh(:,2), ds_caHepPh(:,1), ds_caHepPh(:,3:4)]];
ssNo = 31;
% evolvSpeed = [1 3 7 10];
...
for c1 = 1:length(evolvSpeed)
    ssArray{c1} = snapShot_create(ds_caHepPh, ssNo, evolvSpeed(c1), msConvType);
end

nodesNo = max([ds_caHepPh(:,1); ds_caHepPh(:,2)]);
...
title('caHepPh Dataset Results Using Both Proposed & Traditional Methods');
