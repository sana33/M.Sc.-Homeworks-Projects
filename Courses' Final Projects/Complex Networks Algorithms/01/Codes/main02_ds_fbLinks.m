...
ds_fbLinks = load('ds_fbLinks.mat'); ds_fbLinks = ds_fbLinks.ds_fbLinks; ds_fbLinks = double(ds_fbLinks);
ds_fbLinks = [ds_fbLinks; [ds_fbLinks(:,2), ds_fbLinks(:,1), ds_fbLinks(:,3:4)]];
ssNo = 31;
% evolvSpeed = [1 3 7 10];
...
for c1 = 1:length(evolvSpeed)
    ssArray{c1} = snapShot_create(ds_fbLinks, ssNo, evolvSpeed(c1), msConvType);
end

nodesNo = max([ds_fbLinks(:,1); ds_fbLinks(:,2)]);
...
title('fbLinks Dataset Results Using Both Proposed & Traditional Methods');
