...
ds_enronEmail = load('ds_enronEmail.mat'); ds_enronEmail = ds_enronEmail.ds_enronEmail; ds_enronEmail = double(ds_enronEmail);
ds_enronEmail = [ds_enronEmail; [ds_enronEmail(:,2), ds_enronEmail(:,1), ds_enronEmail(:,3:4)]];
ssNo = 31;
% evolvSpeed = [1 3 7 10];
...
for c1 = 1:length(evolvSpeed)
    ssArray{c1} = snapShot_create(ds_enronEmail, ssNo, evolvSpeed(c1), msConvType);
end

nodesNo = max([ds_enronEmail(:,1); ds_enronEmail(:,2)]);
...
metricType = 'asymmetric'; recurr = 0; selfLoop = 1; ph1SC = 3; passSC = 2;
...
title('enronEmail Dataset Results Using Both Proposed & Traditional Methods');
