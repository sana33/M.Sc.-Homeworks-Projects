...
ds_synthTempSymm = load('ds_synthTempSymm.mat'); ds_synthTempSymm = ds_synthTempSymm.ds_synthTempSymm; ds_synthTempSymm = double(ds_synthTempSymm);
ds_synthTempSymm = [ds_synthTempSymm; [ds_synthTempSymm(:,2), ds_synthTempSymm(:,1), ds_synthTempSymm(:,3:4)]];
ssNo = 31;
% evolvSpeed = [1 3 7 10];
...
for c1 = 1:length(evolvSpeed)
    ssArray{c1} = snapShot_create(ds_synthTempSymm, ssNo, evolvSpeed(c1), msConvType);
end

nodesNo = max([ds_synthTempSymm(:,1); ds_synthTempSymm(:,2)]);
...
metricType = 'symmetric'; recurr = 0; selfLoop = 0; ph1SC = 3; passSC = 5;
...
title('Synthetic Symmetric Dataset Results Using Both Proposed & Traditional Methods');
