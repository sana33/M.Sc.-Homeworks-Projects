...
% Loading dataset synthTempDS
ds_synthTempAsymm = load('ds_synthTempAsymm.mat'); ds_synthTempAsymm = ds_synthTempAsymm.ds_synthTempAsymm; ds_synthTempAsymm = double(ds_synthTempAsymm);
ds_synthTempAsymm = [ds_synthTempAsymm; [ds_synthTempAsymm(:,2), ds_synthTempAsymm(:,1), ds_synthTempAsymm(:,3:4)]];
ssNo = 31;
% evolvSpeed = [1 3 7 10];
...
for c1 = 1:length(evolvSpeed)
    ssArray{c1} = snapShot_create(ds_synthTempAsymm, ssNo, evolvSpeed(c1), msConvType);
end

nodesNo = max([ds_synthTempAsymm(:,1); ds_synthTempAsymm(:,2)]);
...
metricType = 'asymmetric'; recurr = 0; selfLoop = 1; ph1SC = 3; passSC = 5;
...
for c1 = 1:length(evolvSpeed)
    ...
    modulArrProp(1,c1) = modulCalc(A, 'asymmetric', commArray{1,c1});
...
for c1 = 1:length(evolvSpeed)
    for c2 = 2:ssNo
        ...
        modulArrProp(c2,c1) = modulCalc(t1_A, 'asymmetric', commArray{c2,c1});
        [~, commIndsTrad] = BlondelSparse(t1_A, metricType, recurr, selfLoop, ph1SC, passSC);
        commArrTrad = commArrayMaker(commIndsTrad);
        modulArrTrad(c2,c1) = modulCalc(t1_A, 'asymmetric', commArrTrad);
    end
end
...
plot(1:ssNo,modulArrProp(:,1),'b',1:5:ssNo,modulArrProp(1:5:ssNo,1),'bo', ...
    1:ssNo,modulArrTrad(:,1),'g',1:5:ssNo,modulArrTrad(1:5:ssNo,1),'gs'); xlim([1 31]); ylim([-1 1]);
...
title('Synthetic Asymmetric Dataset Results Using Both Proposed & Traditional Methods');

figure;
plot(1:ssNo,modulArrProp(:,2),'b',1:5:ssNo,modulArrProp(1:5:ssNo,2),'bo', ...
    1:ssNo,modulArrTrad(:,2),'g',1:5:ssNo,modulArrTrad(1:5:ssNo,2),'gs'); xlim([1 31]); ylim([-1 1]);
...
plot(1:ssNo,modulArrProp(:,3),'b',1:5:ssNo,modulArrProp(1:5:ssNo,3),'bo', ...
    1:ssNo,modulArrTrad(:,3),'g',1:5:ssNo,modulArrTrad(1:5:ssNo,3),'gs'); xlim([1 31]); ylim([-1 1]);
...
plot(1:ssNo,modulArrProp(:,4),'b',1:5:ssNo,modulArrProp(1:5:ssNo,4),'bo', ...
    1:ssNo,modulArrTrad(:,4),'g',1:5:ssNo,modulArrTrad(1:5:ssNo,4),'gs'); xlim([1 31]); ylim([-1 1]);
