...
%     m_f(:,c1) = [0; 0];
    P_f(:,:,c1) = (repmat(W(c1,:).^alpha,m,1).*(X-repmat(m_f(:,c1),1,n)))*transpose((X-repmat(m_f(:,c1),1,n)))./ ...
        sum(W(c1,:).^alpha);
...
while true
    W_Temp = W;
    for c1 = 1:k
        D(c1,:) = sum(transpose((transpose(X-repmat(m_f(:,c1),1,n))*M_inv(:,:,c1)).*transpose(X-repmat(m_f(:,c1),1,n))));
%         D(c1,:) = sum(transpose((transpose(X-repmat(m_f(:,c1),1,n))/M_inv(:,:,c1)).*transpose(X-repmat(m_f(:,c1),1,n))));
    end
