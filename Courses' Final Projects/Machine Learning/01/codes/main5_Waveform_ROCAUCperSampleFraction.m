...
%% Considering Waveform dataset, according to Figure 7 of the
% paper
...
% Making boxplots contents on Waveform dataset, according to Figure 7 of
% the paper
for c1 = 1:10
    for c2 = 1:25
        while true
            subSampleInd = randperm(dsSize,floor(.1*c1*dsSize));
            if sum(Waveform(subSampleInd,end)) > 1
                break;
            end
        end
        SUBolLabels = Waveform(subSampleInd,end);
        [SUBlofValues, ~, ~] = LocalOutlierFactor(Waveform(subSampleInd,1:end-2), 50, 50, 1);
        [SUBLoOPvalues, ~] = LocalOutlierProb(Waveform(subSampleInd,1:end-2), 50);
...
% Considering Feature Bagging Algorithm on Waveform dataset, according to
% Figure 7 of the paper
[~, ~, ~, aucTotLOF] = FeatureBagging(Waveform(:,1:end-2), olLabels, 'LOF', 25, 'CumulativeSum', 50, 50, 1, 2);
[~, ~, ~, aucTotLoOP] = FeatureBagging(Waveform(:,1:end-2), olLabels, 'LoOP', 25, 'CumulativeSum', 50, 50, 1, 2);

% Considering ROC-AUC for total ensembles on Waveform dataset, according to
% Figure 7 of the paper
...
title('Waveform dataset; BaseMethod: LOF; k = 50');
...
title('Waveform dataset; BaseMethod: LoOP; k = 50');
xlabel('Sample fraction','FontWeight','bold');
ylabel('ROC AUC','FontSize',12,'FontWeight','bold');
ylim([0 1]);
