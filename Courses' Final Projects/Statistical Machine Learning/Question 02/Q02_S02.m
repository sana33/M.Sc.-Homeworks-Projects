...
while indexHigh < length(logValues)-1
    indexHigh = indexLow + windowLength - 1;
    dataOnProcess = logValues(indexLow:indexHigh);
    windowMean = mean(dataOnProcess);
    windowVar = var(dataOnProcess);
    VaRisk = windowMean + sqrt(windowVar).*norminv(alpha);
