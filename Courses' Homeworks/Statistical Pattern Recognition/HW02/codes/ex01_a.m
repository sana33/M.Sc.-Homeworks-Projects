...
% Plotting Bayes Decision Rule with Minimum Prob. of Error
H = @(x1,x2).5.*([x1; x2]-mu1')'*(Sigma1\([x1; x2]-mu1'))-.5.*([x1; x2]-mu2')'*(Sigma2\([x1; x2]-mu2'))+ ...
    .5.*log(det(Sigma1)/det(Sigma2))-log(prior1/prior2);
subplot(1,2,2);
h1 = ezplot(H,[-3,3,-3,3]); grid on; title('Bayes Decision Rule with Minimum Probability of Error');
h1.Color = 'r'; h1.LineWidth = 2;

% syms h(x1,x2);
% h(x1,x2) = .5.*([x1; x2]-mu1')'*(Sigma1\([x1; x2]-mu1'))-.5.*([x1; x2]-mu2')'*(Sigma2\([x1; x2]-mu2'))+ ...
%     .5.*log(det(Sigma1)/det(Sigma2))-log(prior1/prior2);
% figure; fimplicit(h,[-3 3 -3 3]);
