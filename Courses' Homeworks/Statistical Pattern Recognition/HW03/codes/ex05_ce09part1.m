...
% considering classes w1 and w3
W = [ones(1,size(w1,2)) ones(1,size(w3,2)); w1 w3; ones(1,size(w1,2)) -ones(1,size(w3,2))];
WaugNorm = [ones(1,size(w1,2)) -ones(1,size(w3,2)); w1 -w3; ones(1,size(w1,2)) -ones(1,size(w3,2))];
figure;
group = [ones(1,size(w1,2)) 2*ones(1,size(w3,2))];
...
title('Scattered Data from classes w1 and w2 with Plotted Decision Boundaries');
legend('w1','w3','Decision Boundary','location','southeast');

figure;
gscatter(W(2,:),W(3,:),group,'br','so'); grid on; hold on;
h2 = ezplot(@(x1,x2) a(1)+a(2).*x1+a(3).*x2,[min(W(2,:))-1,max(W(2,:))+1,min(W(3,:))-1,max(W(3,:))+1]);
h2.Color = 'r'; h2.LineStyle = '-'; h2.LineWidth = 2;
title('Scattered Data from classes w1 and w3 with Plotted Decision Boundaries');
legend('w1','w3','Decision Boundary','location','southeast');
